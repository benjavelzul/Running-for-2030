...
Be strategic in your movements! Some paths might lead to dead ends, while others hold multiple SDG items. Your ultimate goal is to collect as many different SDG items as possible within the given time or move limit. Good luck, the future of our planet is in your hands!""")
    ...
    class Mazegame:
        ...
        def Menu(self):
            ...
            up_level = Button(self.master, text=">", command=None, bg=self.themes["bg"], fg=self.themes["fg"])
            down_level = Button(self.master, text="<", command=None, bg=self.themes["bg"], fg=self.themes["fg"])
                ...
                for col in range(len(self.maze[row])):
                    ...
                    if self.maze[row][col] == "W":
                        ...
                        if self.mode >= 11:
                            ...
                            self.item_message = self.item_message_list[4]
                            self.time_limit = 200  # Increase time limit for higher levels
                        elif self.mode >= 8:
                            self.time_limit = 150
                            item_image = self.item_list[3]
                            self.item_message = self.item_message_list[3]
                        elif self.mode >= 5:
                            self.time_limit = 120
                            item_image = self.item_list[2]
                            self.item_message = self.item_message_list[2]
                        elif self.mode >= 2:
                            item_image = self.item_list[1]
                            self.item_message = self.item_message_list[1]
                        elif self.mode >= 0:
                            item_image = self.item_list[0]
                            self.item_message = self.item_message_list[0]
                        else:
                            print("Invalid mode")
        ...
        def animate_item(self):
            ...
            self.item_frame_index = (self.item_frame_index + 1) % len(self.item_list[self.mode-1])
            z = 0
            for item in self.items:
